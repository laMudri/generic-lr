...
GraphL = `GraphL ▹ λ where
  (`η∘ s) →   ⟨ [ u1 ∙ wire ]ᶜ `⊢ bundle s ⟩ `⊆ bundle s
  (`μ∘ s) →   ⟨ []ᶜ `⊢ wire ⟩ `✴ ⟨ []ᶜ `⊢ wire ⟩ `✴ ⟨ [ u1 ∙ wire ]ᶜ `⊢ bundle s ⟩
              `⊆ bundle s
  (`η∙ s) →   ⟨ []ᶜ `⊢ wire ⟩ `✴ ⟨ []ᶜ `⊢ bundle s ⟩ `⊆ bundle s
  (`μ∙ s) →   ⟨ []ᶜ `⊢ wire ⟩ `✴ ⟨ [ u1 ∙ wire ]ᶜ ++ᶜ [ u1 ∙ wire ]ᶜ `⊢ bundle s ⟩
              `⊆ bundle s
...
Graph s t = [ GraphL , ∞ ] ctx {s} (λ _ → u1) (λ _ → wire) ⊢ bundle t
\end{code}
%</Graph>
