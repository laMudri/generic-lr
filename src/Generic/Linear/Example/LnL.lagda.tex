...
\begin{code}
  ...
  LnL = `LnL ▹ λ where
    ...
    (`⊗e A B C)  →
      ⟨ []ᶜ `⊢ lin , A t⊗ B ⟩ `✴ ⟨ [ 1# , lin , A ]ᶜ ++ᶜ [ 1# , lin , B ]ᶜ `⊢ lin , C ⟩
                                                                 `⊆ lin , C
    (`⊸i A B)    → ⟨ [ 1# , lin , A ]ᶜ `⊢ lin , B ⟩              `⊆ lin , A t⊸ B
    (`⊸e A B)    →
      ⟨ []ᶜ `⊢ lin , A t⊸ B ⟩ `✴ ⟨ []ᶜ `⊢ lin , A ⟩              `⊆ lin , B
    (`Fi X)      → `□⁰⁺ˣ ⟨ []ᶜ `⊢ int , X ⟩                      `⊆ lin , tF X
    (`Fe X C)    →
      ⟨ []ᶜ `⊢ lin , tF X ⟩ `✴ ⟨ [ ω# , int , X ]ᶜ `⊢ lin , C ⟩  `⊆ lin , C

    `1i          → `□⁰⁺ˣ `⊤                                      `⊆ int , t1
    (`×i X Y)    →
      `□⁰⁺ˣ (⟨ []ᶜ `⊢ int , X ⟩ `∧ ⟨ []ᶜ `⊢ int , Y ⟩)           `⊆ int , X t× Y
    (`×e i X Y)  → `□⁰⁺ˣ ⟨ []ᶜ `⊢ int , X t× Y ⟩  `⊆ int , [ X > i < Y ]
    (`→i X Y)    → `□⁰⁺ˣ ⟨ [ ω# , int , X ]ᶜ `⊢ int , Y ⟩        `⊆ int , X t→ Y
    (`→e X Y)    →
      `□⁰⁺ˣ (⟨ []ᶜ `⊢ int , X t→ Y ⟩ `∧ ⟨ []ᶜ `⊢ int , X ⟩)      `⊆ int , Y
...
\begin{code}
  ...
  pattern 1#i = V.`con (`1i , refl , □ᶜ⟨ _ ⟩ tt)
  ...
  -- annotations are duplicable (i.e, either ω# or 0#).
